...
io.on("connection", (client) => {
	...
	function handleJoinGame(roomName) {
		...
		const new_room = new Room(
			...
			clientRooms[other_client].settings
		);
		clientRooms[client.id] = new_room;
		client.join(roomName);
		client.number = new_room.first_turn == 1 ? 2 : 1;
		client.emit("init", client.number);
		client.emit("settings", clientRooms[other_client].settings);
	...
	//PvP mode.
	function handleNewMatch(settings) {
		...
		if (rooms) {
			for (let key of rooms.keys()) {
				...
				if (rooms.get(key).size == 1) {
					...
					const found_room = clientRooms[room_id];
					if (found_room.locked == 1) continue;
					if (found_room.settings.hasCoords != settings.hasCoords) continue;
					if (found_room.settings.hasPieces != settings.hasPieces) continue;
					const new_room = new Room(
						key,
						0,
						clientRooms[room_id].first_turn,
						settings_room
					);
					clientRooms[client.id] = new_room;
					client.join(key);
		...
		//Create state.
		state[roomName] = new Chess();
		const room = new Room(roomName, 0, null, settings_room);
		clientRooms[client.id] = room;
		client.join(roomName);
		client.number = room.first_turn;
		client.emit("init", room.first_turn);
